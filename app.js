...
const colors = ["red", "blue", "purple", "green", "pink", "lime", "orange"];
...
io.on("connection", function(socket) {
  ...
  socket.on("name", function(data) {
    ...
    colorMappings[id] = randChoice(colors);
    if (notStarted) setup();
    else socket.emit("word", currLetters);
  });
  ...
  socket.on("answer", function(data) {
    ...
    if (msg != "" && msg === chosenWord) {
      ...
      if (correctResponses === numOfGuessers) {
        io.emit("message", {
          msg: `Everyone got the correct answer! The next round will be starting soon...`
        });
        io.emit("showWord", chosenWord);
        clearTimers();
        io.emit("time", 0);
        setTimeout(function() {
          setup();
      ...
      //Upon answering, the time will shorten to start the next round faster
        ...
        io.emit("time", 15);
        io.emit("message", {
          msg: `${nameMappings[socket.id]} got the correct answer!`
        });
      }
    } else {
  ...
  socket.on("disconnect", function() {
    ...
    //Ending the game if there are less than 2 users to play.
    if (activePlayers() < 2) {
      clearTimers();
      io.emit("reset");
      notStarted = true;
...
//Helper Functions
//Chooses a random item from the provided array, specifically current letters or avaliable clients or colors
function randChoice(array) {
  let rIndex = randIndex(array.length);
  let randChoice = array[rIndex];
  if (array === words) {
    currLetters = [];
    ...
    if (activePlayers() > 1) {
      ...
      globalTimer = setInterval(function() {
        if (time === 1) {
          ...
          io.emit("showWord", chosenWord);
          io.emit("message", {
            msg: "The next round will be starting soon..."
          });
          setTimeout(function() {
            setup();
          }, 5000);
